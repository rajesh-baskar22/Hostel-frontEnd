    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-blue-50 rounded-xl shadow-lg p-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <h1 className="text-3xl font-bold text-blue-900 flex items-center gap-4">
          ...
          <div className="mt-4 md:mt-0 p-4 bg-blue-100 rounded-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <p className="font-semibold text-blue-900">Room #{room.roomNumber}</p>
                <p className="text-blue-800">Type: {room.type}</p>
                <p className="text-blue-800">Capacity: {room.capacity}</p>
              </div>
              <div className="space-y-2">
                <p className="text-blue-800">Status: <span className="font-medium text-blue-700">{room.availabilityStatus}</span></p>
                <p className="text-blue-800">Occupied: {room.occupied}/{room.capacity}</p>
                <p className="text-blue-800">Features: {room.features?.AC ? "AC, " : ""}{room.features?.WIFI ? "WiFi" : ""}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-blue-900 mb-2">Check In Date</label>
              <input
                type="date"
                value={bookingData.checkInDate}
                onChange={(e) => setBookingData({ ...bookingData, checkInDate: e.target.value })}
                className="w-full px-4 py-2 rounded-lg border border-blue-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white"
                required
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-blue-900 mb-2">Check Out Date</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-sm font-medium text-blue-900 mb-2">Number of Occupants</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm font-medium text-blue-900 mb-2">Select Resident</label>
              <select
